...
// Les annonces sont téléchargées depuis le backend API via `fetch` dans le provider.
...
export function AppProvider({ children }) {
  ...
  // Fetch from Python Backend FastApi
  useEffect(() => {
    fetch('http://localhost:5001/api/properties')
      .then(res => res.json())
      .then(data => {
        // Optionnel: filtrer ou retravailler les données reçues si nécessaire
        // Les champs de l'API correspondent très bien au format attendu (prix, prixM2, surface, type, quartier, etc.)
        setAnnonces(data);
      })
      .catch(err => console.error("Could not load backend annonces:", err));
  }, []);
